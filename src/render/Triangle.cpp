...
constexpr std::array<Vertex, 3> vertices{{
    ...
    {{-0.65f, 0.55f}, {0.0f, 0.0f, 1.0f}}
}};
}

void Triangle::initialize(QVulkanWindow* window) {
    window_ = window;
    ...
    if (memoryType == UINT32_MAX) qFatal("No host-visible coherent memory for triangle.");
...
VkShaderModule Triangle::loadShader(const QString& path) {
    QFile file(path);
...
void Triangle::createPipeline() {
    const auto vertex = loadShader(QStringLiteral(":/shaders/triangle.vert.spv"));
    const auto fragment = loadShader(QStringLiteral(":/shaders/triangle.frag.spv"));
    ...
    check(result, "Create triangle pipeline");
}

void Triangle::draw(VkCommandBuffer command) {
    const auto size = window_->swapChainImageSize();
    const float side = float(std::min(size.width(), size.height()));
    // Keep proportions when resizing, before camera matrices arrive in Q3.
    VkViewport viewport{(float(size.width())-side)/2, (float(size.height())-side)/2, side, side, 0, 1};
    ...
    functions_->vkCmdDraw(command, 3, 1, 0, 0);
}

void Triangle::releasePipeline() {
