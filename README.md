...
第一阶段本地运行包已验收，见 [验收记录](docs/First-stage-acceptance.md) 和 [运行包说明](docs/First-stage-run-package.md)。使用 `scripts/package.ps1` 重新生成 ZIP，`scripts/test-package.ps1` 检查解压后的实际程序。
...
打开工程后自动取景，首页“显示全部”可找回整个场景，见 [自动取景说明](docs/Scene-framing-learning.md)。

已完成 1,000 对象双显卡性能基线与拾取优化，见 [性能测试说明](docs/Q6-performance-learning.md)。

已加入透视/正交切换与顶视、前视、右视，见 [视图操作与学习说明](docs/Q6-views-learning.md)。
...
顶部已加入首页 / 模型 / 视图 / 工程四个可切换功能区。首页选择左键旋转、平移、缩放模式，模型页输入基本体尺寸后在地面单击创建，视图页独立控制网格、世界轴和方向立方体，工程页提供打开、保存与另存为。详见 [功能区操作与源码说明](docs/Ribbon-learning.md) 与 [创建放置说明](docs/Q5-creation-learning.md)。Q4 场景基础见 [Q4 学习说明](docs/Q4-learning.md)。

双击 `scripts/run.bat` 运行。默认显示一个彩色长方体和一个青色圆柱体，状态栏显示实际显卡和验证层状态。左键单击模型或左侧名称进行选择，右侧输入尺寸、位置或旋转后按回车或移开焦点应用；空白单击或 Esc 取消选择。变换输入框支持 Ctrl 步进：位置 100 mm、旋转 15°。模型页支持创建基本体，放置时显示线框预览，Ctrl 对齐到 100 mm 地面网格，Esc 取消放置。左键拖动按首页模式导航，中键拖动绕模型观察，Shift + 中键平移，滚轮缩放。在视口中按 F 重置，空格切换场景预览旋转；放置期间暂时禁用预览旋转。创建和参数修改支持撤销/重做；切换场景示例会重新载入预设并清空历史；如有未保存修改，会先提示保存、不保存或取消。
